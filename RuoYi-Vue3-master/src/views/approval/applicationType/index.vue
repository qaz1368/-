    ...
    <el-row :gutter="20">
      ...
      <el-col :span="24">
        ...
        <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange" class="full-width-table">
          ...
          <el-table-column label="更新时间" align="center" prop="updatedAt" v-if="columns[3].visible" width="300">
            <template #default="scope">
          ...
          <el-table-column label="操作" align="center" width="200" class-name="small-padding fixed-width">
            <template #default="scope">
              <el-tooltip content="修改" placement="top">
                <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['system:tag:edit']"></el-button>
              </el-tooltip>
              <el-tooltip content="删除" placement="top">
                <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['system:tag:remove']"></el-button>
              </el-tooltip>
            </template>
          </el-table-column>
        </el-table>
...
import {
  addApplicationType,
  deleteApplicationType, deleteApplicationTypeList,
  getApplicationTypeById, listApplicationType, updateApplicationType
} from "../../../api/application/applicationType";
...
function getList() {
  loading.value = true;
  listApplicationType(proxy.addDateRange(queryParams.value, dateRange.value)).then(res => {
    loading.value = false;
...
function handleDeleteList() {
  ...
  const applicationTypeIds = selectedRows.map(row => row.applicationTypeId);
  proxy.$modal.confirm(`是否确认删除标签编号为"${applicationTypeIds}"的数据项？`).then(function () {
    return deleteApplicationTypeList(applicationTypeIds);
  }).then(() => {
    getList();
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除标签编号为"' + row.applicationTypeId + '"的数据项？').then(function () {
    return deleteApplicationType(row.applicationTypeId);
  }).then(() => {
    getList();
    proxy.$modal.msgSuccess("删除成功");
  }).catch(() => {});
...
function handleExport() {
  proxy.download("/application-type/export", {
    ...queryParams.value,
  },`application-type_${new Date().getTime()}.xlsx`);
};
/** 用户状态修改  */
...
function handleUpdate(row) {
  ...
  getApplicationTypeById(row.applicationTypeId).then(response => {
    form.value = response;
...
function submitForm() {
  proxy.$refs["userRef"].validate(valid => {
    if (valid) {
      if (form.value.applicationTypeId != undefined) {
        updateApplicationType(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addApplicationType(form.value).then(response => {
