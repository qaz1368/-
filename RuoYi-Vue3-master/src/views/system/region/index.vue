...
function getList() {
  loading.value = true;
  listRegion(proxy.addDateRange(queryParams.value, dateRange.value)).then(res => {
    loading.value = false;
...
function handleDeleteList() {
  ...
  const regionIds = selectedRows.map(row => row.regionId);
  proxy.$modal.confirm(`是否确认删除标签编号为"${regionIds}"的数据项？`).then(function () {
    return deleteRegionList(regionIds);
  }).then(() => {
    getList();
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除标签编号为"' + row.regionId + '"的数据项？').then(function () {
    return deleteRegion(row.regionId);
  }).then(() => {
    getList();
    proxy.$modal.msgSuccess("删除成功");
  }).catch(() => {});
...
function handleUpdate(row) {
  ...
  getRegionById(row.regionId).then(response => {
    form.value = response;
